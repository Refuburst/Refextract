...
#         results, auth_year = process_other_functions(pdf_path)
...
def check_citation_presence(row, text):
    ...
    if count_citation_1 + count_citation_2 == 0:
        row['Number of times Cited'] = 'Ref not cited/Havard'
    else:
        row['Number of times Cited'] = count_citation_1 + count_citation_2

    return row

def generate_harvard_citation(row, count):
    authors = row['author']
    year = row['year'][0]

    if any('et al.' in author for author in authors):
        return f"{' and '.join(authors)} ({year})"
    elif count == 1:
        surname = re.findall(r'[A-Z][a-z]+', authors[0])[0]
...
def process_other_functions(text):
    ...
    if text:
        ...
        references = refextract.extract_references_from_file(text_filename)
    else:
        return "Text data is empty or invalid."

    with open('extracted_text.txt', 'w', encoding='utf-8', errors='ignore') as text_file:
        text_file.write(text)
    ...
    try:
        df['Authors'] = df['author'].apply(lambda x: len(re.findall(r'[A-Z][a-z]+', x)))
        df['Harvard Citation 1'] = df.apply(generate_harvard_citation, axis=1, count=1)
        df['Harvard Citation 2'] = df.apply(generate_harvard_citation, axis=1, count=2)
        df = df.apply(check_citation_presence, axis=1, args=(text,))

        auth_count = df.to_html(classes='table table-stripped').encode('utf-8').decode('utf-8')
    except UnicodeEncodeError as e:
        auth_count = f"Error encoding HTML: {e}"

    cited_count = (df['Number of times Cited'] != 'Ref not cited/Havard').sum()
    not_cited_count = (df['Number of times Cited'] == 'Ref not cited/Havard').sum()
    total_count = len(df)
    percent_cited = (cited_count / total_count) * 100
    percent_not_cited = (not_cited_count / total_count) * 100
    ...
    marks_df = summary_df.to_html(classes='table table-stripped')
    combined_df = pd.concat([summary_df, df], ignore_index=True)
    combined_df.to_csv('reference_data.csv', index=False)

    return {
        'Alphabet Check': "References in the PDF are in alphabetical order" if df['author'].is_monotonic_increasing else "References in the PDF are not in alphabetical order",
...
def process_cite_functions(pdf_path):
    ...
    def extract_author_and_year(citation):
        ...
        form = 'end' if (citation[0] == '(') else 'in text'
    ...
    def cite_sentence_position(citation):
        ...
        for cit in citations:
            if citation_text.startswith(cit):
                position = "At Start Please check!!"
            elif citation_text.endswith(cit):
                position = "✔"
            else:
                position = "✔"
        return {'In Sentence Use': position}

    dfc[['In Sentence Use']] = dfc['Citations'].apply(cite_sentence_position).apply(pd.Series)

    specific_pattern = r"([A-Z][a-z]+) et al\.,? \(?(\d{4})\)?"
    generic_pattern = r"[A-Z][a-z]+ (?:et\,?\s?al\.,?|et al\.,?|et\,? al\.,?) \d{4}"

    def check_et_al_syntax(citation):
        return '✔' if re.search(specific_pattern, citation) else '❌' if re.search(generic_pattern, citation) else 'n/a'

    dfc['et_al syntax'] = dfc['Citations'].apply(check_et_al_syntax)

    citedf = dfc.to_html(classes='table table-stripped')
    dfc.to_csv('citation_data.csv', index=False)
...
@app.route('/downloadcite_csv_file')
def downloadcite_csv_file():
    csv_file_path = 'citation_data.csv'
    return send_file(csv_file_path, as_attachment=True)
